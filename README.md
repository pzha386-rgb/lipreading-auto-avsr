...
## Models & Data (not included in this repo)

Large binaries and licensed datasets are intentionally excluded. To reproduce:

| Asset | Size | Where to get it |
|---|---|---|
| `vsr_trlrs2lrs3vox2avsp_base.pth` (main VSR checkpoint, 3,291 h) | ~956 MB | [Auto-AVSR Model Zoo](https://github.com/mpc001/auto_avsr#model-zoo) |
| `vsr_trlrs3vox2_base.pth` (weaker ckpt, 1,759 h) | ~890 MB | same Model Zoo |
| `auto_avsr` codebase | — | `git clone https://github.com/mpc001/auto_avsr` |
| LRS3 test set (1,321 utterances) | ~1 GB | Original distribution discontinued; we used a HuggingFace mirror of the preprocessed test split. LRS3 licence does not permit redistribution. |
| BBC out-of-domain test set (91 clips) | ~40 MB | Not redistributed (source video copyright). Rebuild with `scripts/`: cut clips → `extract_rois.py` → `convert_bbc_manifest.py`. Manifest format example: `scripts/bbc_test_example.csv`. |
| facexlib detection/alignment weights | ~290 MB | Auto-downloaded on first run of `roi_utils.py` |

Expected layout on the GPU host: everything under `/data/default/` — `auto_avsr/`, `*.pth`, `lrs3_dataset/`, `bbc_test/` (see `eval/EVAL.md`).
